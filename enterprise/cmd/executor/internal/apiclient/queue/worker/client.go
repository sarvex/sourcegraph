...
func (c *Client) Dequeue(ctx context.Context, workerHostname string, extraArguments any) (job executor.Job, _ bool, err error) {
	ctx, _, endObservation := c.operations.dequeue.With(ctx, &err, observation.Args{LogFields: []otlog.Field{
		otlog.String("queueName", c.client.Queue()),
	}})
	defer endObservation(1, observation.Args{})

	req, err := c.client.NewJSONRequest(http.MethodPost, fmt.Sprintf("%s/dequeue", c.client.Queue()), executor.DequeueRequest{
...
func (c *Client) MarkComplete(ctx context.Context, jobID int) (_ bool, err error) {
	...
	req, err := c.client.NewJSONRequest(http.MethodPost, fmt.Sprintf("%s/markComplete", c.client.Queue()), executor.MarkCompleteRequest{
	...
	return true, c.client.DoAndDrop(ctx, req)
}

func (c *Client) MarkErrored(ctx context.Context, jobID int, errorMessage string) (_ bool, err error) {
	...
	req, err := c.client.NewJSONRequest(http.MethodPost, fmt.Sprintf("%s/markErrored", c.client.Queue()), executor.MarkErroredRequest{
		JobOperationRequest: executor.JobOperationRequest{
			ExecutorName: c.options.ExecutorName,
			JobID:        jobID,
		},
...
func (c *Client) MarkFailed(ctx context.Context, jobID int, errorMessage string) (_ bool, err error) {
	...
	req, err := c.client.NewJSONRequest(http.MethodPost, fmt.Sprintf("%s/markFailed", c.client.Queue()), executor.MarkErroredRequest{
		JobOperationRequest: executor.JobOperationRequest{
			ExecutorName: c.options.ExecutorName,
			JobID:        jobID,
		},
...
func (c *Client) CanceledJobs(ctx context.Context, knownIDs []int) (canceledIDs []int, err error) {
	req, err := c.client.NewJSONRequest(http.MethodPost, fmt.Sprintf("%s/canceledJobs", c.client.Queue()), executor.CanceledJobsRequest{
...
func (c *Client) Ping(ctx context.Context, jobIDs []int) (err error) {
	req, err := c.client.NewJSONRequest(http.MethodPost, fmt.Sprintf("%s/heartbeat", c.client.Queue()), executor.HeartbeatRequest{
		ExecutorName: c.options.ExecutorName,
	})
	if err != nil {
		return err

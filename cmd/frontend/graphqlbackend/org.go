...
func (r *schemaResolver) CreateOrganization(ctx context.Context, args *struct {
	...
	if envvar.SourcegraphDotComMode() && args.StatsID != nil {
		...
		if err != nil {
			r.logger.Warn("Cannot update orgs open beta stats", logger.String("id", *args.StatsID), logger.Int32("orgID", newOrg.ID), logger.Error(err))
		}
	}
...
func (r *schemaResolver) RemoveUserFromOrganization(ctx context.Context, args *struct {
	...
	r.logger.Info("removing user from org", logger.Int32("userID", userID), logger.Int32("orgID", orgID))
